...
class ContentAnalyzer:
    ...
    def _analyze_language_indicators(text: str) -> Dict[str, float]:
        ...
        # English indicators
        english_common = len(re.findall(r'\b(the|and|or|is|are|was|were|to|of|in|for|with)\b', text, re.IGNORECASE))
    ...
    @staticmethod
    def suggest_processing_method(analysis_result, text_metrics: Optional[TextMetrics] = None) -> Dict[str, str]:
        ...
        if text_metrics and text_metrics.language_indicators.get('special_chars', 0) > 0.1:
            if suggestions["method"] == "direct_extraction":
                suggestions.update({
    ...
    def _calculate_ocr_quality_score(text: str, words: List[str], lines: List[str]) -> float:
        ...
        common_english_words = {
            'the', 'and', 'or', 'is', 'are', 'was', 'were', 'to', 'of', 'in', 'for', 'with',
            'that', 'this', 'have', 'has', 'had', 'will', 'would', 'could', 'should',
            'not', 'but', 'from', 'they', 'we', 'he', 'she', 'it', 'you', 'me', 'us',
            'all', 'any', 'some', 'each', 'every', 'other', 'than', 'only', 'such',
            'court', 'case', 'law', 'order', 'appeal', 'petition', 'plaintiff', 'defendant'
        }
