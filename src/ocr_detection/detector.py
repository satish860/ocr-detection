...
class PDFAnalyzer:
    ...
    def get_summary(self, results: Optional[List[AnalysisResult]] = None) -> Dict[str, Any]:
        ...
        type_percentages = {
            page_type: (count / total_pages) * 100 
        ...
        return {
            ...
            "recommended_action": self._get_recommendation_enhanced(type_counts, total_pages, avg_text_quality, results),
            "problematic_pages": problematic_pages
        }
    
    def _analyze_images(self, page) -> Dict[str, Any]:
    ...
    def _classify_page_enhanced(self, text_ratio: float, image_ratio: float, 
                              text_length: int, content_image_count: int, text_metrics, 
                              background_ratio: float) -> tuple[PageType, float]:
        ...
        if text_length > 200:  # Reasonable amount of text
            ...
            text_density = text_metrics.text_density
            formatting_score = text_metrics.formatting_consistency
    ...
    def _classify_page(self, text_ratio: float, image_ratio: float, 
                      text_length: int, image_count: int) -> tuple[PageType, float]:
    ...
    def _get_recommendation_enhanced(self, type_counts: Dict[str, int], total_pages: int, 
                                   avg_text_quality: float, results: List[AnalysisResult]) -> str:
        ...
        for result in results:
            ...
            if (result.text_length > 200 and 
                result.details.get("text_quality", {}).get("ocr_quality_score", 0) > 0.6):
                good_text_pages += 1
        ...
        if good_text_percentage > 60 and avg_text_quality > 0.6:
            return "NO OCR NEEDED"
        elif text_percentage > 70 and avg_text_quality > 0.5:
            return "NO OCR NEEDED" 
        elif good_text_percentage > 30 and avg_text_per_page > 300:
            return "NO OCR NEEDED"
        elif scanned_percentage > 70:
            return "OCR REQUIRED"
        elif mixed_percentage > 80 and avg_text_quality < 0.4:
            return "OCR REQUIRED"
        elif text_percentage < 10 and scanned_percentage + mixed_percentage > 80:
            return "OCR REQUIRED"
        else:
    ...
    def _identify_problematic_pages(self, results: List[AnalysisResult]) -> Dict[str, Any]:
        ...
        # Create summary
        total_needing_ocr = len(scanned_pages) + len(mixed_pages_needing_ocr) + len(low_quality_text_pages)
        ...
        return {
            ...
            "summary": self._create_page_summary(scanned_pages, mixed_pages_needing_ocr, low_quality_text_pages, empty_pages)
        }
    
    def _create_page_summary(self, scanned_pages: List[int], mixed_pages: List[Dict], 
                           low_quality_pages: List[Dict], empty_pages: List[int]) -> str:
