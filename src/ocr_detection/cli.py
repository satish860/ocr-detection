...
from .detector import AnalysisResult, PDFAnalyzer
...
@click.option(
    "--format",
    "-f",
    type=click.Choice(["json", "csv", "text", "summary"]),
    default="summary",
    ...
    try:
        with PDFAnalyzer(pdf_path) as analyzer:
            ...
            if page is not None:
                ...
                click.echo(
                    f"Analyzing {total_pages} pages of {pdf_path.name} using {method} processing..."
                )
                ...
                if verbose and start_time:
                    ...
                    click.echo(
                        f"Analysis completed in {elapsed:.2f} seconds ({pages_per_second:.1f} pages/sec)"
                    )
...
def _print_summary(
    ...
    if problematic.get("total_pages_needing_ocr", 0) > 0:
        ...
        click.echo(
            f"Total pages that would benefit from OCR: {problematic['total_pages_needing_ocr']}"
        )
    ...
    if low_confidence_pages:
        ...
        for result in low_confidence_pages:
            click.echo(
                f"  Page {result.page_number + 1}: {result.page_type.value} "
...
def _format_text_output(
    results: list[AnalysisResult], summary: dict[str, Any], verbose: bool, include_text: bool
) -> str:
...
def _format_json_output(
    results: list[AnalysisResult], summary: dict[str, Any], include_text: bool
) -> str:
...
def _write_or_print(content: str, output_path: Path | None = None, mode: str = "text") -> None:
